...
if (isset($_POST['answer'])){
    ...
    while($post=mysqli_fetch_assoc($posts)){
        $answer=mysqli_query($connect,"SELECT * FROM `std_answer_1`where std_id ='$_SESSION[user_id]' and test_number='$test_num'and question_number='$num'");
        $answer=mysqli_fetch_assoc($answer);
        if($answer==true){
            $a = $_POST['a'.$num.''];
            $insert=mysqli_query( $connect ,"UPDATE `std_answer_1` SET `answer_1`='$a' WHERE  std_id = '$_SESSION[user_id]' and test_number='$test_num'and question_number='$num'and answer_number='$num'");
            if ($post['right_answer']==$a){
                $insert=mysqli_query( $connect ,"UPDATE `std_answer_1` SET `degree`='1' WHERE  std_id = '$_SESSION[user_id]' and test_number='$test_num'and question_number='$num'");

            }
            else{
                $insert=mysqli_query( $connect ,"UPDATE `std_answer_1` SET `degree`='0' WHERE  std_id = '$_SESSION[user_id]' and test_number='$test_num'and question_number='$num'");
            }
        }
        else{
            $a=$_POST['a'.$num.''];
            $insert=mysqli_query( $connect ,"INSERT INTO `std_answer_1`( 
                                                                                    `std_id`,
                                                                                     `test_number`,
                                                                                      `question_number`,
                                                                                       `answer_number`, 
            ...
            if ( $post['right_answer']==$a){
                $insert=mysqli_query( $connect ,"UPDATE `std_answer_1` SET `degree`='1' WHERE  std_id = '$_SESSION[user_id]' and test_number='$test_num'and question_number='$num'and answer_number='$num'");
            }else{
                $insert=mysqli_query( $connect ,"UPDATE `std_answer_1` SET `degree`='0' WHERE  std_id = '$_SESSION[user_id]' and test_number='$test_num'and question_number='$num'and answer_number='$num'");
    ...
    session_write_close();
    echo'<meta http-equiv="refresh" content="1;\'show_answer_1.php?code='.$codee.'&id='.$test_num.'\'"/>';
