...
for ($i=1; $i<=40 ; $i++ ){
    ...
    $insert=mysqli_query( $connect ,"SELECT * FROM std_answer_2 where std_id ='$_SESSION[user_id]' and test_number='$test_num'and question_number='$i' ");
    while ( $answer=mysqli_fetch_assoc($insert)){
        $degree=$degree + $answer['degree'];

    }
}
$std_degree=mysqli_query( $connect ,"SELECT * FROM `student_2_degree` where stud_id ='$_SESSION[user_id]' and test_number='$test_num' and stud_level='2'");
$update_std_degree=mysqli_fetch_assoc($std_degree);
if($update_std_degree==true){
    $update_degree=mysqli_query( $connect ,"UPDATE `student_2_degree` SET `degree`='$degree' WHERE  stud_id = '$_SESSION[user_id]' and test_number='$test_num' and stud_level='2'");
}
else{
    $insert=mysqli_query( $connect ,"INSERT INTO `student_2_degree`( `stud_id`,`stud_level`, `test_number`, `degree`) VALUES ('$_SESSION[user_id]','2','$test_num','$degree')");
}
?>
<!doctype html>
