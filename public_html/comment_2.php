...
if(isset($_POST['add_comment'])){
    ...
    if(empty($comment)){
        ...
        $add_comment = "INSERT INTO `comment_for_2`( `comment_writer`,`writer_id`, `comment`, `date`) VALUES ('$_SESSION[user]','$_SESSION[user_id]','$comment','$date')";
...
if (isset($_POST['all_replay'])){
    ...
    if(empty($replay_cod)){
        ...
        $add_replay = "INSERT INTO `replay_for_2`(`replay_CODE`, `replay`) VALUES ('$replay_cod','$replay')";
...
if (isset($_POST['add_comm'])){
    ...
    if(empty($replay_1)){
        ...
        $add_replay_1 = "INSERT INTO `replay_for_2`(`replay_CODE`, `replay`) VALUES ('$replay_cod_1','$replay_1')";
...
if (isset($_POST['delete'])){
    ...
    $add_replay = "DELETE FROM `comment_for_2` WHERE comment_id ='$replay_cod_2'";
    $insert_replay = mysqli_query($connect, $add_replay);
    if (isset( $insert_replay)){
...
if ($_SESSION['status']=='admin'){
    ...
    $comm=mysqli_query( $connect ,"SELECT * FROM comment_for_2 ");
    while($comm_2=mysqli_fetch_assoc($comm)){
        $re=mysqli_query( $connect ,"SELECT * FROM `replay_for_2`where replay_CODE='$comm_2[comment_id]'");
        echo '
<div class="card col-lg-12 m-2 p-2 bg-light" dir="rtl" align="right">
  ...
  <button type="delete" name="delete" id="delete" value="'.$comm_2['comment_id'].'" class="btn btn-danger mb-2 mr-2">ارسال الرد</button>
    ...
    $comm=mysqli_query( $connect ,"SELECT * FROM comment_for_2 where writer_id='$_SESSION[user_id]'");
    while($comm_2=mysqli_fetch_assoc($comm)){
        $re=mysqli_query( $connect ,"SELECT * FROM `replay_for_2`where replay_CODE='$comm_2[comment_id]'");
        echo '
