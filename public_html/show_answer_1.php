...
for ($i=1; $i<=40 ; $i++ ){
    ...
    $insert=mysqli_query( $connect ,"SELECT * FROM std_answer_1 where std_id ='$_SESSION[user_id]' and test_number='$test_num'and question_number='$i' ");
    while ( $answer=mysqli_fetch_assoc($insert)){
        $degree=$degree + $answer['degree'];

    }
}
$std_degree=mysqli_query( $connect ,"SELECT * FROM `student_2_degree` where stud_id ='$_SESSION[user_id]' and test_number='$test_num' and stud_level='1'");
$update_std_degree=mysqli_fetch_assoc($std_degree);
if($update_std_degree==true){
    $update_degree=mysqli_query( $connect ,"UPDATE `student_2_degree` SET `degree`='$degree' WHERE  stud_id = '$_SESSION[user_id]' and test_number='$test_num' and stud_level='1'");
}
else{
    $insert=mysqli_query( $connect ,"INSERT INTO `student_2_degree`( `stud_id`,`stud_level`, `test_number`, `degree`) VALUES ('$_SESSION[user_id]','1','$test_num','$degree')");
}
?>
<!doctype html>
...
<div class="container mb-5" style="padding-top: 150px">
    <div class="col-md-12" style=" border: solid #000000 1px ; border-radius: 10px">
        <div class="form-area " >
            <h3 class="font-weight-bold"><?php echo $test_num;?> اجابة اختبار الصف الاول   الثانوي رقم </h3>
            ...
            elseif ($code_data['code_status']=='disable'){
                ...
                $posts=mysqli_query( $connect ,"SELECT * FROM test  where `level`='1'and `test_num` = '$test_num' ");
                while($post=mysqli_fetch_assoc($posts)){
                    $insert=mysqli_query( $connect ,"SELECT * FROM std_answer_1 where std_id ='$_SESSION[user_id]' and test_number='$test_num'and question_number='$num' ");
...
include_once("include/footer.php");
?>
<!-- jQuery first, then Popper.js, then Bootstrap JS -->  <script src="js/jquery-3.3.1.min.js"></script>
<script src="js/jquery-migrate-3.0.1.min.js"></script>
<script src="js/jquery-ui.js"></script>
<script src="js/popper.min.js"></script>
<script src="js/bootstrap.min.js"></script>
